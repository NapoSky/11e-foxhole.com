...
// InlineDescriptionContent defined directly within this file
const InlineDescriptionContent: React.FC = () => {
  const { description, actions } = descriptionData;
...
const Description: React.FC = () => {
  const { imageSrc } = descriptionData;
  ...
  return (
    <Section noPadding sectionId={SectionId.Description}>
      <div className="relative flex min-h-screen w-full items-center justify-center px-2 sm:px-4">
        <Image
          alt={`${descriptionData.name}-image`}
          className="absolute z-0 h-full w-full object-cover fill"
          placeholder="blur"
          priority
          src={imageSrc}
