...
const OfficerSection: FC = memo(() => {
  ...
  }, [activeIndex, setTestimonial, officers.length]);
  ...
  // If no officers, don't render the section
  if (!officers.length) {
    return null;
  }

  return (
    <Section noPadding sectionId={SectionId.Officers}>
      <div
        ...
        <div className="z-10 w-full max-w-screen-md px-4 lg:px-0">
          <div className="flex flex-col items-center gap-y-6 rounded-xl bg-gray-800/60 p-6 shadow-lg">
            <div
              ...
              ref={scrollContainer}>
              {officers.map((testimonial, index) => {
                const isActive = index === activeIndex;
                return (
                  <Officer isActive={isActive} key={`${testimonial.name}-${index}`} officer={testimonial} />
                );
              })}
            </div>
            <div className="flex gap-x-4">
              {[...Array(officers.length)].map((_, index) => {
                const isActive = index === activeIndex;
                return (
                  <button
                    className={classNames(
...
const Officer: FC<{officer: Officers; isActive: boolean}> = memo(
  ({officer: {text, name, image}, isActive}) => (
    <div
      className={classNames(
